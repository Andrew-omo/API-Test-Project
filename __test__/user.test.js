...
import { createUsers, updateUsers, patchUsers, invalidUserInputData } from "../helper/user-helper";
...
describe('POST/ Add new userId to Tests', () =>{
  ...
  it('POST/ Add a new User',async()=>{
     ...
     .set('Authorization', `Bearer ${TOKEN}`).send(createUsers)
     console.log(response.body)
  ...
  it('Get a single User/:id',async()=>{
    const response = await router.get(`/users/${userId}`)
 ...
 it('PUT/ udate user Email :id',async()=>{
  const response = await router.put(`/users/${userId}`)
  .set('Authorization', `Bearer ${TOKEN}`).send(updateUsers)
  console.log(response.body)
  expect(response.status).toBe(200);
  expect(response.body).not.toEqual({});
  expect(response.body).toMatchObject({data:{}})
 ...
 it('PATCH/ udate user Name',async()=>{
  const response = await router.put(`/users/${userId}`)
  .set('Authorization', `Bearer ${TOKEN}`).send(patchUsers)
  console.log(response.body)
  expect(response.status).toBe(200);
  expect(response.body).not.toEqual({});
  expect(response.body).toMatchObject({data:{}})
 ...
 it('Delect User/:id',async()=>{
  const response = await router.delete(`/users/${userId}`)
  .set('Authorization', `Bearer ${TOKEN}`)
  console.log(response.body)
...
describe('Negative Test', ()=>{
 ...
 it('404 The request does not exist',async()=>{
  const response = await router.post('/user')
  .set('Authorization', `Bearer ${TOKEN}`)
  .send(createUsers)
  console.log(response.body)
 ...
 it('401/ Authentication Failed',async()=>{
  ...
  expect(response.body.data.message).toEqual('Authentication failed');
